...
def test_td0021_c11_discovery_surfaces_mode_workbench_and_logical_ref_debug_state(
  ...
  _make_valid_config_folder(
    ...
    product_root=PRODUCT_ROOT,
    guide_refs=(
      "resource.authoritative_guide.ci_authoring_authoritative_guides_ci_authoring",
    ),
  )
...
def test_doctor_reports_invalid_configuration_bootstrap_drift_and_invalid_bootstrap_shapes(
  ...
  assert any(
    finding["subject"] == "authoritative_refs.product_root"
  ...
  assert any(
    "bootstrap configuration is invalid:" in finding["message"]
    for finding in invalid_yaml_report["findings"]
  )
