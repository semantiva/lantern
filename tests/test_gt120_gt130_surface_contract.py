...
def test_no_lantern_change_index_pseudo_path_in_shipped_guides() -> None:
    ...
    assert offending_files == [], "Lantern/change/INDEX.md pseudo-path found in runtime-shipped files: " + str(
        offending_files
    )
...
def test_no_lantern_path_semantics_note_in_ev_templates() -> None:
    ...
    assert (
        disclaimer_fragment not in ev_gt120
    ), "GT-120 EV template still contains Lantern/... path-semantics disclaimer"
    assert (
        disclaimer_fragment not in ev_gt130
...
def test_readme_uses_package_install_for_grammar() -> None:
    ...
    assert (
        "pip install -e ../lantern-grammar" not in readme
...
def test_conftest_has_no_sibling_grammar_probe() -> None:
    ...
    assert (
        "_CANDIDATE_GRAMMAR_PATHS" not in conftest
    ), "conftest.py still defines _CANDIDATE_GRAMMAR_PATHS sibling-probe list"
    assert "lantern-grammar" not in conftest, "conftest.py still references lantern-grammar sibling path"
