...
_ARTIFACT_ID_RE = re.compile(
  r"\b(?:INI|DIP|SPEC|ARCH|CH|TD|DC|DB|CI|EV|DEC|IS|Q)-\d{4}(?:-[0-9a-fA-F-]{36})?\b"
)
_INDEX_LINK_RE = re.compile(
  r"\((?P<path>(?:arch|ch|ci|db|dc|dec|dip|ev|ini|is|q|spec|td)/[^)]+\.md)\)"
)
_ENTITY_ORDER = {
  "artifact": 0,
  "status": 1,
    ...
    return [
      {
        ...
        "status": None,
        "heading_labels": (),
        "source_path": (config_surface.folder / "main.yaml").relative_to(config_surface.folder.parent.parent).as_posix(),
        "fields": {
    ...
    records.append(
      {
        ...
        "mode_id": None,
        "direct_refs": (),
        "gate_names": tuple(sorted(_workbench_gate_names(workbench["lifecycle_placement"]))) if workbench else (),
      }
    )
  return records
...
def _load_workbench_overrides(
  ...
  for override in config_surface.workbench_overrides:
    ...
    overrides[override.workbench_id] = yaml.safe_load(
